...
import com.qa.pojo.LMSManualFeesRoot;
import com.qa.pojo.ManualFees;
...
public class Core_LMS_RestAssured_POST extends TestBase {

	public Core_LMS_RestAssured_POST() throws IOException {
		super();
	...
	public void verifySuccessResponse() throws JsonProcessingException
	{
		row = excelHelper.getCellRowNum(sheetName, "TCID", "TC1");
		assignTCDescription(row);
		getExceldata(row);
		//for GET call below func is used
		//getParamExcelData(row);
		//for POST call below func is used
		getJSONBody(row);
	...
	public String getJSONBody(int row) throws JsonProcessingException
	{
		ManualFees manualFees = new ManualFees(234,543);
		LMSManualFeesRoot manualFeesRoot = new LMSManualFeesRoot(manualFees);
		ObjectMapper mapper = new ObjectMapper();
		strPayLoad = mapper.writeValueAsString(manualFeesRoot);
		return strPayLoad;
	}
